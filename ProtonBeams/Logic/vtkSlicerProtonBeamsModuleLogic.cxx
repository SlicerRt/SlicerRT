...
//----------------------------------------------------------------------------
vtkStandardNewMacro(vtkSlicerProtonBeamsModuleLogic);

//----------------------------------------------------------------------------
vtkSlicerProtonBeamsModuleLogic::vtkSlicerProtonBeamsModuleLogic()
{
  this->ProtonBeamsNode = NULL;
}

//----------------------------------------------------------------------------
vtkSlicerProtonBeamsModuleLogic::~vtkSlicerProtonBeamsModuleLogic()
{
  vtkSetAndObserveMRMLNodeMacro(this->ProtonBeamsNode, NULL);
}

//----------------------------------------------------------------------------
void vtkSlicerProtonBeamsModuleLogic::PrintSelf(ostream& os, vtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//----------------------------------------------------------------------------
void vtkSlicerProtonBeamsModuleLogic::SetAndObserveProtonBeamsNode(vtkMRMLProtonBeamsNode *node)
{
  vtkSetAndObserveMRMLNodeMacro(this->ProtonBeamsNode, node);
}

//---------------------------------------------------------------------------
void vtkSlicerProtonBeamsModuleLogic::SetMRMLSceneInternal(vtkMRMLScene * newScene)
{
...
//-----------------------------------------------------------------------------
void vtkSlicerProtonBeamsModuleLogic::RegisterNodes()
{
  ...
  scene->RegisterNodeClass(vtkSmartPointer<vtkMRMLProtonBeamsNode>::New());
}

//---------------------------------------------------------------------------
void vtkSlicerProtonBeamsModuleLogic::UpdateFromMRMLScene()
{
...
//---------------------------------------------------------------------------
void vtkSlicerProtonBeamsModuleLogic::OnMRMLSceneNodeAdded(vtkMRMLNode* node)
{
  ...
  if (node->IsA("vtkMRMLAnnotationFiducialNode") || node->IsA("vtkMRMLProtonBeamsNode"))
  {
    this->Modified();
  }
}
...
//---------------------------------------------------------------------------
void vtkSlicerProtonBeamsModuleLogic::OnMRMLSceneNodeRemoved(vtkMRMLNode* node)
{
  if (!node || !this->GetMRMLScene())
  {
    return;
...
//---------------------------------------------------------------------------
void vtkSlicerProtonBeamsModuleLogic::OnMRMLSceneEndImport()
{
  // If we have a parameter node select it
  vtkMRMLProtonBeamsNode *paramNode = NULL;
  vtkMRMLNode *node = this->GetMRMLScene()->GetNthNodeByClass(0, "vtkMRMLProtonBeamsNode");
  if (node)
  {
    paramNode = vtkMRMLProtonBeamsNode::SafeDownCast(node);
    vtkSetAndObserveMRMLNodeMacro(this->ProtonBeamsNode, paramNode);
  }
}

//---------------------------------------------------------------------------
void vtkSlicerProtonBeamsModuleLogic::OnMRMLSceneEndClose()
{
  this->Modified();
}

//---------------------------------------------------------------------------
void vtkSlicerProtonBeamsModuleLogic::ComputeSourceFiducialPosition(std::string &errorMessage, vtkTransform* aIsocenterToSourceTransform/*=NULL*/)
{
  if (!this->ProtonBeamsNode || !this->GetMRMLScene())
  {
    return;
  }
  if ( !this->ProtonBeamsNode->GetIsocenterFiducialNodeId()
    || !strcmp(this->ProtonBeamsNode->GetIsocenterFiducialNodeId(), "") )
  {
  ...
  vtkMRMLAnnotationFiducialNode* isocenterNode = vtkMRMLAnnotationFiducialNode::SafeDownCast(
    this->GetMRMLScene()->GetNodeByID(this->ProtonBeamsNode->GetIsocenterFiducialNodeId()) );
  if (!isocenterNode)
  {
  ...
  if (SlicerRtCommon::IsStringNullOrEmpty(this->ProtonBeamsNode->GetSourceFiducialNodeId()))
  {
    errorMessage = "Empty source fiducial node ID!";
    vtkErrorMacro(<<errorMessage); 
    return;
  }
  if (!strcmp(this->ProtonBeamsNode->GetSourceFiducialNodeId(), this->ProtonBeamsNode->GetIsocenterFiducialNodeId()))
  {
  ...
  vtkMRMLAnnotationFiducialNode* sourceNode = vtkMRMLAnnotationFiducialNode::SafeDownCast(
    this->GetMRMLScene()->GetNodeByID(this->ProtonBeamsNode->GetSourceFiducialNodeId()) );
  if (!sourceNode)
  {
...
//---------------------------------------------------------------------------
void vtkSlicerProtonBeamsModuleLogic::CreateProtonBeamModel(std::string &errorMessage)
{
  if (!this->ProtonBeamsNode || !this->GetMRMLScene())
  {
    return;
  ...
  if ( SlicerRtCommon::IsStringNullOrEmpty(this->ProtonBeamsNode->GetIsocenterFiducialNodeId())
    || SlicerRtCommon::IsStringNullOrEmpty(this->ProtonBeamsNode->GetSourceFiducialNodeId()) )
  {
  ...
  // Get beam model node
  if ( !this->ProtonBeamsNode->GetBeamModelNodeId()
    || !strcmp(this->ProtonBeamsNode->GetBeamModelNodeId(), "") )
  {
  ...
  vtkMRMLModelNode* beamModelNode = vtkMRMLModelNode::SafeDownCast(
    this->GetMRMLScene()->GetNodeByID(this->ProtonBeamsNode->GetBeamModelNodeId()) );
  if (!beamModelNode)
  {
  ...
  displayNode->SetOpacity(this->ProtonBeamsNode->GetBeamModelOpacity());
