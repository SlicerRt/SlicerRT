...
void qSlicerDeformationFieldVisualizerModuleWidget::setMRMLScene(vtkMRMLScene* scene){
	...
	if (scene &&  d->logic()->GetParameterNode() == 0){
...
void qSlicerDeformationFieldVisualizerModuleWidget::onEnter(){
	...
	vtkMRMLDeformationFieldVisualizerParametersNode* pNode = d->logic()->GetParameterNode();
	...
	if (pNode == NULL){
		...
		if (node){
			...
			d->logic()->SetAndObserveParameterNode(pNode);
			return;
		}
		else{
			...
			d->logic()->SetAndObserveParameterNode(newNode);
		}
	}
...
void qSlicerDeformationFieldVisualizerModuleWidget::setDeformationFieldVisualizerParametersNode(vtkMRMLNode *node){
	...
	qvtkReconnect( d->logic()->GetParameterNode(), pNode, vtkCommand::ModifiedEvent, this, SLOT(updateWidgetFromMRML()));
