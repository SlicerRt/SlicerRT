...
/*
//----------------------------------------------------------------------------
void vtkSlicerPlastimatchProtonDoseEngine::InitializeAccumulatedDose(Plm_image::Pointer plmRef)
{
...
//----------------------------------------------------------------------------
void vtkSlicerPlastimatchProtonDoseEngine::CalculateDose(
  ...
  double RxDose)
{
  ...
  std::cout << "MIN %f AVE %f MAX %f NONZERO %d NUMVOX %d" << std::endl, 
  ...
  try
  {
    ...
    std::cout << "Reference dose position: %lg %lg %lg" << std::endl, rt_plan.get_ref_dose_point()[0], rt_plan.get_ref_dose_point()[1], rt_plan.get_ref_dose_point()[2]);
    ...
    std::cout << "Dose prescription = %lg" << std::endl, rt_plan.get_normalization_dose());
    ...
    std::cout << "Source position: %lg %lg %lg" << std::endl, rt_beam->get_source_position()[0], rt_beam->get_source_position()[1], rt_beam->get_source_position()[2]);
    std::cout << "Setting isocenter position -> ";
    rt_beam->set_isocenter_position(isocenter);
    std::cout << "Isocenter position: %lg %lg %lg" << std::endl, rt_beam->get_isocenter_position()[0], rt_beam->get_isocenter_position()[1], rt_beam->get_isocenter_position()[2]);
    ...
    std::cout << "Algorithm Flavor = %c" << std::endl, rt_beam->get_flavor());
    ...
    std::cout << "Beam weight = %lg" << std::endl, rt_beam->get_beam_weight());
    std::cout << "Setting smearing -> ";
    rt_beam->set_smearing(protonBeamNode->GetRangeCompensatorSmearingRadius());
    std::cout << "Smearing = %lg" << std::endl, rt_beam->get_smearing());
    ...
    std::cout << "Source size = %lg" << std::endl, rt_beam->get_source_size());
    std::cout << "Setting step length -> ";
    rt_beam->set_step_length(protonBeamNode->GetStepLength());
    std::cout << "Step length = %lg" << std::endl, rt_beam->get_step_length());
    ...
    std::cout << "Aperture distance = %lg" << std::endl, rt_beam->get_aperture()->get_distance());
    std::cout << "Setting aperture origin -> ";
    rt_beam->get_aperture()->set_origin(protonBeamNode->GetApertureOrigin());
    std::cout << "Aperture origin = %lg %lg" << std::endl, protonBeamNode->GetApertureOrigin(0), protonBeamNode->GetApertureOrigin(1));
    std::cout << "Setting aperture spacing -> ";
    rt_beam->get_aperture()->set_spacing(protonBeamNode->GetApertureSpacing());
    std::cout << "Aperture Spacing = %lg %lg" << std::endl, rt_beam->get_aperture()->get_spacing(0),  rt_beam->get_aperture()->get_spacing(1));
    std::cout << "Setting aperture dim -> ";
    rt_beam->get_aperture()->set_dim(protonBeamNode->GetApertureDim() );
    std::cout << "Aperture dim = %d %d" << std::endl, rt_beam->get_aperture()->get_dim(0), rt_beam->get_aperture()->get_dim(1) );
    ...
    std::cout << "Manual energy prescription set to %d" << std::endl, rt_beam->get_mebs()->get_have_prescription());
    ...
    if (!rt_plan.prepare_beam_for_calc (rt_beam))
    {
      ...
      std::cerr << "Sorry, rt_plan.prepare_beam_for_calc() failed." << std::endl;
      return;
    }
...
//---------------------------------------------------------------------------
itk::Image<float, 3>::Pointer vtkSlicerPlastimatchProtonDoseEngine::GetRangeCompensatorVolume()
{
  vtkErrorMacro("Obsolete method!");
...
//---------------------------------------------------------------------------
itk::Image<unsigned char, 3>::Pointer vtkSlicerPlastimatchProtonDoseEngine::GetApertureVolume()
{
  vtkErrorMacro("Obsolete method!");
...
//---------------------------------------------------------------------------
itk::Image<float, 3>::Pointer vtkSlicerPlastimatchProtonDoseEngine::GetComputedDose()
{
  vtkErrorMacro("Obsolete method!");
...
//---------------------------------------------------------------------------
itk::Image<float, 3>::Pointer vtkSlicerPlastimatchProtonDoseEngine::GetAccumulatedDose()
{
  vtkErrorMacro("Obsolete method!");
