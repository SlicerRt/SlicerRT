...
//---------------------------------------------------------------------------
template <typename Key> bool SlicerRtCommon::OrderPlanesAlongNormal( std::map<Key, vtkPlane*> inputPlanes, std::map<Key, int>& outputPlaneIdentifierOrdering )
{
  std::map<Float64, vtkVector3<Float64> > intermediateSortMap;
  // Iterate over each plane
  vtkVector3<Float64> lastNormal;
  for(  std::map<Key, vtkPlane*>::iterator it = inputPlanes.begin(); it != inputPlanes.end(); ++it )
  {
    // Calculate the projection of the origin onto the normal
    vtkVector3<Float64> origin(it->second->GetOrigin()[0], it->second->GetOrigin()[1], it->second->GetOrigin()[2]);
    vtkVector3<Float64> normal(it->second->GetNormal()[0], it->second->GetNormal()[1], it->second->GetNormal()[2]);

    if( it != inputPlanes.begin() && ( normal.X() != lastNormal.X() || normal.Y() != lastNormal.Y() || normal.Z() != lastNormal.Z() ) )
    {
      return false;
    }
    Float64 mag = origin.Dot(normal);
    intermediateSortMap[mag] = origin;

    lastNormal.Set(normal.X(), normal.Y(), normal.Z());
  }

  // Now, do a key lookup from the origin of the plane and sort
  int i = 0;
  outputPlaneIdentifierOrdering.clear();
  for (std::map<Float64, vtkVector3<Float64> >::iterator it = intermediateSortMap.begin(); it != intermediateSortMap.end(); ++it)
  {
    Key identifier;
    // Find the identifier that belongs to this plane by comparing origins
    for( std::map<Key, vtkPlane*>::iterator lookupIt = inputPlanes.begin(); lookupIt != inputPlanes.end(); ++lookupIt )
    {
      if( lookupIt->second->GetOrigin()[0] == it->second.X() && lookupIt->second->GetOrigin()[1] == it->second.Y() && lookupIt->second->GetOrigin()[2] == it->second.Z() )
      {
        identifier = lookupIt->first;
        break;
      }
    }

    // Transfer the ordering back to the slices vector
    outputPlaneIdentifierOrdering[identifier] = i;
    i++;
  }
